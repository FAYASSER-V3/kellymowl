...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'owner', fromMe: true, desc: 'shows the detail of bot owner'}, (async (message, match) => {
        ...
        if (Config.OWNER == 'default') {
            await message.client.sendMessage(message.jid,'Kellymowl created by *FAYAS SER*' , MessageType.text);
        }
        else {
    ...
    Asena.addCommand({pattern: 'owner', fromMe: false, desc: 'shows the detail of bot owner'}, (async (message, match) => {
        ...
        if (Config.OWNER == 'default') {
            ...
            await message.client.sendMessage(message.jid,'ʜʏ ᴅᴜᴅᴇ....👋🏻\n\n```MY OWNER:``` *'+Config.OWNER + '*\n\n ```OWNER NUMBER``` *'+Config.BSL + '\n\n---------------------', MessageType.text);
        }
    }));
}
